...
def gconnect():
    ...
    output += '<img src="'
    output += login_session['picture']
    output += ' " style = "width: 50%; height: 50%;border-radius: 150px;-webkit-border-radius: 150px;-moz-border-radius: 150px;"> '
...
def itemsJSON(category_id):
	items=session.query(Item).join(Category).filter(Category.id==category_id).all()
...
def newCategory():
	if 'username' not in login_session:
		return "<script>function myFunction() {\
					alert('You are not authenticated to access this page. Please authenticate yourself.');\
				 	window.location.href = '/login';}</script><body onload='myFunction()''>"
...
def newItem():
	...
	if request.method == 'POST':
		...
		addItem = Item(name=request.form['title'], description=request.form['description'], 
...
def editItem(item_id):
	...
	if login_session['user_id'] != item.user_id:
		return "<script>function myFunction() {\
			alert('You are not authorized to edit this item. Please edit what you created.');\
		 	window.location.href = '/catalog';}</script><body onload='myFunction()''>"
	if request.method == 'POST':
		editedItem.name=request.form['name']
		editedItem.description=request.form['description']
		selectedCategory=request.form['category']
		selectedCategory_id=selectedCategory.split('.')[0]
		editedItem.category_id=selectedCategory_id
