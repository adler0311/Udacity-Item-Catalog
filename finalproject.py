...
def fbconnect():
    ...
    url = 'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret\
    ...
    # Get user picture
    url = 'https://graph.facebook.com/v2.4/me/picture?%s&redirect=0&heigh\
            t=200&width=200' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    data = json.loads(result)
...
def gconnect():
    ...
    output += '<img src="'
    output += login_session['picture']
    output += ' " style = "width: 20%; height: 20%;border-radius: \
                100%;-webkit-border-radius: 100%;-moz-border-radius: 100%;"> '
...
def editItem(item_id):
    ...
    if login_session['user_id'] != item.user_id:
        return "<script>function myFunction() {\
            ...
            Please edit what you created.');\
            window.location.href = '/catalog';}</script><body onload=\
            'myFunction()''>"
    if request.method == 'POST':
